...
const Register = () => {
  ...
  const handleSubmit = (event, errors) => {
      ...
      }).then(resp => resp.json).then((resp) => { window.location.pathname  = "/login" })
    // if (errors && !errors.length) {
    //   useJwt
    //     .register({ username, email, password, country, walletAddress })
    //     .then(res => {
    //       if (res.data.error) {
    //         const arr = {}
  ...
  const handlewalletAddressChange = e => {
    const errs = errors
    if (errs.walletAddress) delete errs.walletAddress
        ...
        <Col className='d-flex align-items-center auth-bg px-2 p-lg-5' lg='4' sm='12'>
          <Col className='px-xl-2 mx-auto' sm='8' md='6' lg='12'>
            ...
            <AvForm action='/' className='auth-register-form mt-2' onSubmit={handleSubmit}>
              ...
              <FormGroup>
                <Label className='form-label' for='register-walletAddress'>
                  WalletAddress
                </Label>
                <AvInput
                  required
                  ...
                  id='register-walletAddress'
                  name='register-walletAddress'
                  value={walletAddress}
                  onChange={handlewalletAddressChange}
                  className={classnames({ 'border-danger': Object.keys(errors).length && errors.walletAddress })}
                />
                {Object.keys(errors).length && errors.walletAddress ? (
                  <small className='text-danger'>{errors.walletAddress}</small>
                ) : null}
              </FormGroup>
              <Row>
                <Col xl="6" md="6" xs="12">
                ...
                <Col xl="6" md="6" xs="12">
                <FormGroup>
                <Label className='form-label' for='register-companyName'>
                  Company
                </Label>
                <AvInput
                  type='text'
              ...
              <Button.Ripple
                block
                color='primary'
                disabled={!email.length || !password.length || !username.length || !terms || !country.length || !lastname.length || password !== confirmPassword || !walletAddress.length }
              >
                Sign up
              </Button.Ripple>
            </AvForm>
